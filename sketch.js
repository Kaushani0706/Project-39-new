...
function preload(){
  ...
  coin4 = loadImage("assets/coin4.png");
}
...
function spawnCoin(){
  if(frameCount % 120 === 0){
    var coin = createSprite(camera.position.x+400,300,45,15);

    coin.velocityX = -(7 + 4*reward/100);
    coin.scale = 0.6;

    var rand = Math.round(random(1,4));
    switch(rand){
      case 1: coin1.addImage(coin1);
              break;
      case 2: coin2.addImage(coin2);
              break;
      case 3: coin3.addImage(coin3);
              break;
      case 4: coin4.addImage(coin4);
              break;
      default: break;
    }
    ...
    coin.setCollider("circle",0,0,coin.width/3,coin.height/3);
    coinsGroup.add(coin);
